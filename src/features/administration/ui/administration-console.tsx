...
import { ConfiguredField, FieldHelpProvider } from "@/shared/ui/field-help";
...
export function AdministrationConsole() {
  ...
  // L'aide par champ est chargée une fois pour toute la console : les
  // descripteurs sont les mêmes d'un onglet à l'autre, et ils ne doivent pas
  // repartir sur le réseau à chaque frappe.
  return <FieldHelpProvider><section className="admin-console app-fullscreen">
    ...
    <div className="admin-content">
      {tab === "game" && <AdminSection icon={Building2} eyebrow="Univers global" title="Jeu, histoire et catégories">
        <div className="admin-grid">
          <Field label="Nom du jeu" path="game.name"><input value={document.game.name} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, game: { ...value.document.game, name: event.target.value } } }))} /></Field>
          <Field label="Type d’organisation" path="organizationType"><select value={document.organizationType} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, organizationType: event.target.value as typeof document.organizationType } }))}><option>School</option><option>Company</option><option>TrainingProvider</option><option>Community</option><option>Custom</option></select></Field>
          <Field label="Nom de l’organisation" path="organization.name"><input value={document.organization.name} onChange={(event) => updateOrganization("name", event.target.value, update)} /></Field>
          <Field label="Description de l’organisation" path="organization.description"><input value={document.organization.description} onChange={(event) => updateOrganization("description", event.target.value, update)} /></Field>
          <Field label="Description" wide path="game.description"><textarea value={document.game.description} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, game: { ...value.document.game, description: event.target.value } } }))} /></Field>
          <Field label="Histoire globale" wide path="game.globalStory"><textarea className="large" value={document.game.globalStory} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, game: { ...value.document.game, globalStory: event.target.value } } }))} /></Field>
        </div>
        <h3>Structure de l’organisation</h3>
        <p className="admin-note">Composez librement établissements, campus, classes et groupes, ou entreprises, départements et équipes. Le parent construit la hiérarchie.</p>
        <div className="config-cards">{[...document.organization.units].sort((left, right) => left.order - right.order).map((unit) => <article key={unit.id}>
          ...
          <div className="admin-grid"><Field label="Type" path="organization.units[].type"><input value={unit.type} onChange={(event) => updateOrganizationUnit(unit.id, "type", event.target.value, update)} /></Field><Field label="Code" path="organization.units[].code"><input value={unit.code} onChange={(event) => updateOrganizationUnit(unit.id, "code", event.target.value, update)} /></Field></div>
          <Field label="Nom de l’unité" path="organization.units[].name"><input value={unit.name} onChange={(event) => updateOrganizationUnit(unit.id, "name", event.target.value, update)} /></Field>
          <Field label="Description de l’unité" path="organization.units[].description"><textarea value={unit.description} onChange={(event) => updateOrganizationUnit(unit.id, "description", event.target.value, update)} /></Field>
          <Field label="Parent" path="organization.units[].parentId"><select value={unit.parentId ?? ""} onChange={(event) => updateOrganizationUnit(unit.id, "parentId", event.target.value || undefined, update)}><option value="">Racine</option>{document.organization.units.filter((candidate) => candidate.id !== unit.id).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}</select></Field>
        </article>)}</div>
      ...
      {tab === "player" && <AdminSection icon={Sparkles} eyebrow="Flux joueur piloté par le moteur" title="Introduction, tutoriel et aide">
        ...
        <div className="admin-grid">
          <Field label="Introduction active" path="intro.enabled"><label className="switch"><input type="checkbox" checked={document.intro.enabled} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, intro: { ...value.document.intro, enabled: event.target.checked } } }))} /><span /></label></Field>
          <Field label="Politique d’affichage" path="intro.displayPolicy"><select value={document.intro.displayPolicy} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, intro: { ...value.document.intro, displayPolicy: event.target.value as typeof document.intro.displayPolicy } } }))}><option>EveryLaunch</option><option>OncePerVersion</option><option>FirstInstall</option></select></Field>
          <Field label="Scénario du mode démo" path="demo.scenarioSlug"><input value={document.demo.scenarioSlug} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, demo: { ...value.document.demo, scenarioSlug: event.target.value } } }))} /></Field>
          <Field label="Durée cible de la démo" path="demo.targetMinutes"><input type="number" min="1" max="120" value={document.demo.targetMinutes} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, demo: { ...value.document.demo, targetMinutes: Number(event.target.value) } } }))} /></Field>
        </div>
        <div className="config-cards">{document.intro.scenes.map((scene, index) => <article key={scene.id}><input value={scene.eyebrow} aria-label="Sur-titre" onChange={(event) => update((value) => ({ ...value, document: { ...value.document, intro: { ...value.document.intro, scenes: value.document.intro.scenes.map((item) => item.id === scene.id ? { ...item, eyebrow: event.target.value } : item) } } }))} /><input value={scene.title} aria-label="Titre" onChange={(event) => update((value) => ({ ...value, document: { ...value.document, intro: { ...value.document.intro, scenes: value.document.intro.scenes.map((item) => item.id === scene.id ? { ...item, title: event.target.value } : item) } } }))} /><textarea value={scene.body} aria-label="Texte" onChange={(event) => update((value) => ({ ...value, document: { ...value.document, intro: { ...value.document.intro, scenes: value.document.intro.scenes.map((item) => item.id === scene.id ? { ...item, body: event.target.value } : item) } } }))} /><Field label="Image HTTPS" path="intro.scenes[].imageUrl"><input value={scene.imageUrl ?? ""} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, intro: { ...value.document.intro, scenes: value.document.intro.scenes.map((item) => item.id === scene.id ? { ...item, imageUrl: event.target.value || undefined } : item) } } }))} /></Field><small>Scène {index + 1}</small></article>)}</div>
        ...
        <div className="admin-grid"><Field label="Version du tutoriel" path="onboarding.version"><input type="number" min="1" value={document.onboarding.version} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, onboarding: { ...value.document.onboarding, version: Number(event.target.value) } } }))} /></Field><Field label="Fréquence par défaut du compagnon" path="assistantPolicy.defaultFrequency"><input type="range" min="0" max="5" value={document.assistantPolicy.defaultFrequency} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, assistantPolicy: { ...value.document.assistantPolicy, defaultFrequency: Number(event.target.value) } } }))} /><small>{document.assistantPolicy.defaultFrequency}/5</small></Field></div>
        <div className="config-cards">{document.onboarding.steps.sort((left, right) => left.order - right.order).map((step) => <article key={step.id}><input value={step.title} aria-label="Titre de l’étape" onChange={(event) => update((value) => ({ ...value, document: { ...value.document, onboarding: { ...value.document.onboarding, steps: value.document.onboarding.steps.map((item) => item.id === step.id ? { ...item, title: event.target.value } : item) } } }))} /><textarea value={step.body} aria-label="Texte de l’étape" onChange={(event) => update((value) => ({ ...value, document: { ...value.document, onboarding: { ...value.document.onboarding, steps: value.document.onboarding.steps.map((item) => item.id === step.id ? { ...item, body: event.target.value } : item) } } }))} /><Field label="Cible UI" path="onboarding.steps[].target"><input value={step.target} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, onboarding: { ...value.document.onboarding, steps: value.document.onboarding.steps.map((item) => item.id === step.id ? { ...item, target: event.target.value } : item) } } }))} /></Field></article>)}</div>
        <h3>Centre d’aide</h3>
      ...
      {tab === "catalog" && <AdminSection icon={LibraryBig} eyebrow="Architecture de l’expérience" title="Parcours, catégories et affectations">
        <p className="admin-note">Les parcours ordonnent des catégories réutilisables. Chaque catégorie référence les scénarios publiés qui lui appartiennent.</p>
        <h3>Parcours</h3><div className="config-cards">{document.journeys.map((journey, index) => <article key={journey.id}><button className="icon-danger" aria-label={`Supprimer ${journey.name}`} onClick={() => update((value) => ({ ...value, document: { ...value.document, journeys: value.document.journeys.filter((item) => item.id !== journey.id), assignments: value.document.assignments.filter((item) => item.contentId !== journey.id) } }))}><Trash2 /></button><Field label="Nom du parcours" path="journeys[].name"><input value={journey.name} onChange={(event) => updateJourney(index, "name", event.target.value, update)} /></Field><Field label="Description du parcours" path="journeys[].description"><textarea value={journey.description} onChange={(event) => updateJourney(index, "description", event.target.value, update)} /></Field><Field label="Image HTTPS" path="journeys[].imageUrl"><input value={journey.imageUrl ?? ""} onChange={(event) => updateJourney(index, "imageUrl", event.target.value, update)} /></Field><div className="permission-picker">{document.categories.map((category) => <label key={category.id}><input type="checkbox" checked={journey.categoryIds.includes(category.id)} onChange={() => update((value) => ({ ...value, document: { ...value.document, journeys: value.document.journeys.map((item) => item.id === journey.id ? { ...item, categoryIds: item.categoryIds.includes(category.id) ? item.categoryIds.filter((id) => id !== category.id) : [...item.categoryIds, category.id] } : item) } }))} /><span><strong>{category.name}</strong><small>{category.description}</small></span></label>)}</div></article>)}</div>
        ...
        <h3>Catégories</h3><div className="config-cards">{document.categories.map((category, index) => <article key={category.id}><button className="icon-danger" aria-label={`Supprimer ${category.name}`} onClick={() => update((value) => ({ ...value, document: { ...value.document, categories: value.document.categories.filter((item) => item.id !== category.id), journeys: value.document.journeys.map((journey) => ({ ...journey, categoryIds: journey.categoryIds.filter((id) => id !== category.id) })), assignments: value.document.assignments.filter((item) => item.contentId !== category.id) } }))}><Trash2 /></button><span className="category-gem" data-accent={category.accent} /><input value={category.name} onChange={(event) => updateCategory(index, "name", event.target.value, update)} /><textarea value={category.description} onChange={(event) => updateCategory(index, "description", event.target.value, update)} /><Field label="Image HTTPS" path="categories[].imageUrl"><input value={category.imageUrl ?? ""} onChange={(event) => updateCategory(index, "imageUrl", event.target.value, update)} /></Field><Field label="IDs scénarios (séparés par des virgules)" path="categories[].scenarioIds[]"><input value={category.scenarioIds.join(", ")} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, categories: value.document.categories.map((item) => item.id === category.id ? { ...item, scenarioIds: event.target.value.split(",").map((id) => id.trim()).filter(Boolean) } : item) } }))} /></Field></article>)}</div>
      ...
      {tab === "language" && <AdminSection icon={Languages} eyebrow="Aucun texte imposé" title="Libellés et vocabulaire du jeu">
        <p className="admin-note">Chaque texte est identifié par une clé stable et publié avec le jeu. Les clients utilisent ces valeurs avec un fallback sûr.</p>
        ...
        {familiar && <div className="role-builder"><h3>Nom et personnalité du familier par défaut</h3><div className="admin-grid"><Field label="Nom affiché" path="familiars[].name"><input value={familiar.name} onChange={(event) => updateFamiliar("name", event.target.value, update)} /></Field><Field label="Description" path="familiars[].description"><input value={familiar.description} onChange={(event) => updateFamiliar("description", event.target.value, update)} /></Field></div></div>}
      </AdminSection>}
      ...
      {tab === "access" && <AdminSection icon={ShieldCheck} eyebrow="RBAC explicable" title="Rôles et permissions">
        ...
        <div className="role-builder"><h3><ShieldCheck /> Affecter un rôle</h3><p className="admin-note">Utilisez l’identifiant stable du compte. Le scope optionnel prépare les périmètres école, classe, entreprise ou équipe.</p><div className="admin-grid"><Field label="ID utilisateur"><input value={assignmentUserId} onChange={(event) => setAssignmentUserId(event.target.value)} placeholder="UUID du compte" /></Field><Field label="Rôle"><select value={assignmentRoleId} onChange={(event) => setAssignmentRoleId(event.target.value)}><option value="">Sélectionner…</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></Field><Field label="Scope optionnel"><input value={assignmentScope} onChange={(event) => setAssignmentScope(event.target.value)} placeholder="ex. class:6e-a" /></Field></div><button className="button button--primary" disabled={busy || !assignmentUserId || !assignmentRoleId} onClick={assignRole}><ShieldCheck /> Affecter</button></div>
      </AdminSection>}
      {tab === "users" && <AdminSection icon={Users} eyebrow="Identité exploitable" title={`${usersTotal} utilisateurs`}><div className="admin-search"><Search /><input value={userQuery} onChange={(event) => setUserQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void load(); }} placeholder="Rechercher par identifiant…" /><button className="button button--quiet" onClick={load}>Rechercher</button></div><div className="user-table">{users.map((user) => <article key={user.id}><div className="user-avatar">{user.userName.slice(0, 2).toUpperCase()}</div><div><strong>{user.userName}</strong><small>{user.externalProvider ? `Microsoft · ${user.externalProvider}` : "Compte local"}</small><div className="permission-pills">{user.roleAssignments.map((assignment) => <span key={`${assignment.roleId}-${assignment.scope}`}>{assignment.roleName} · {assignment.scope}</span>)}</div></div><span className={user.isActive ? "user-state is-active" : "user-state"}>{user.isActive ? "Actif" : "Désactivé"}</span><div className="user-actions"><button className="button button--quiet" onClick={() => mutateUser(user, "toggle")}>{user.isActive ? "Désactiver" : "Réactiver"}</button><button className="icon-danger" aria-label={`Supprimer ${user.userName}`} onClick={() => mutateUser(user, "delete")}><Trash2 /></button></div></article>)}</div></AdminSection>}
      {tab === "ai" && <AdminSection icon={Bot} eyebrow="Provider & compagnon" title="IA et expérience du familier">
        {foundry && <div className="provider-card"><div className="provider-heading"><span className="azure-mark">A</span><div><strong>Azure AI Foundry</strong><small>OpenAI v1 · DefaultAzureCredential</small></div><label className="switch"><input type="checkbox" checked={foundry.enabled} onChange={(event) => update((value) => ({ ...value, document: { ...value.document, aiProviders: value.document.aiProviders.map((item) => item.id === foundry.id ? { ...item, enabled: event.target.checked } : item) } }))} /><span /></label></div><div className="admin-grid"><Field label="Endpoint" path="aiProviders[].endpoint"><input value={foundry.endpoint} onChange={(event) => updateProvider(foundry.id, "endpoint", event.target.value, update)} /></Field><Field label="Déploiement" path="aiProviders[].deployment"><input value={foundry.deployment} onChange={(event) => updateProvider(foundry.id, "deployment", event.target.value, update)} /></Field><Field label="Référence de secret" path="aiProviders[].secretReference"><input value={foundry.secretReference ?? ""} onChange={(event) => updateProvider(foundry.id, "secretReference", event.target.value, update)} /></Field><Field label="Authentification" path="aiProviders[].authentication"><input value={foundry.authentication} disabled /></Field></div></div>}
        {familiar && <div className="familiar-admin">{familiar.portraitUrl ? <img className="familiar-preview" src={familiar.portraitUrl} alt={familiar.name} /> : <div className="familiar-orb">✦</div>}<div><p className="eyebrow">Profil par défaut</p><h3>{familiar.name}</h3><p>{familiar.description}</p></div><div className="admin-grid"><Field label="Portrait HTTPS" path="familiars[].portraitUrl"><input value={familiar.portraitUrl ?? ""} onChange={(event) => updateFamiliar("portraitUrl", event.target.value, update)} /></Field><Field label="Avatar HTTPS" path="familiars[].avatarUrl"><input value={familiar.avatarUrl ?? ""} onChange={(event) => updateFamiliar("avatarUrl", event.target.value, update)} /></Field><Field label="Arrière-plan HTTPS" path="familiars[].backgroundUrl"><input value={familiar.backgroundUrl ?? ""} onChange={(event) => updateFamiliar("backgroundUrl", event.target.value, update)} /></Field><Field label="Licence" path="familiars[].license"><input value={familiar.license ?? ""} onChange={(event) => updateFamiliar("license", event.target.value, update)} /></Field><Field label="Attribution" path="familiars[].attribution"><input value={familiar.attribution ?? ""} onChange={(event) => updateFamiliar("attribution", event.target.value, update)} /></Field><Field label="Style d’écriture" path="familiars[].writingStyle"><input value={familiar.writingStyle} onChange={(event) => updateFamiliar("writingStyle", event.target.value, update)} /></Field><Field label="Niveau d’aide" path="familiars[].helpLevel"><input type="range" min="0" max="5" value={familiar.helpLevel} onChange={(event) => updateFamiliar("helpLevel", Number(event.target.value), update)} /><small>{familiar.helpLevel}/5</small></Field></div></div>}
      </AdminSection>}
      {tab === "auth" && <AdminSection icon={KeyRound} eyebrow="Accès cumulatif ou exclusif" title="Authentification">
        <div className="choice-cards">{(["LocalOnly", "Cumulative", "EntraOnly"] as const).map((mode) => <button key={mode} className={document.authentication.mode === mode ? "is-selected" : ""} onClick={() => update((value) => ({ ...value, document: { ...value.document, authentication: { ...value.document.authentication, mode, localEnabled: mode !== "EntraOnly", entraEnabled: mode !== "LocalOnly" } } }))}><Check /> <strong>{mode === "LocalOnly" ? "Compte local" : mode === "EntraOnly" ? "Microsoft uniquement" : "Les deux méthodes"}</strong><small>{mode}</small></button>)}</div>
        <div className="admin-grid"><Field label="Tenant Entra ID" path="authentication.entraTenantId"><input value={document.authentication.entraTenantId ?? ""} onChange={(event) => updateAuth("entraTenantId", event.target.value, update)} /></Field><Field label="Client ID" path="authentication.entraClientId"><input value={document.authentication.entraClientId ?? ""} onChange={(event) => updateAuth("entraClientId", event.target.value, update)} /></Field></div>
      </AdminSection>}
 ...
 *
 * Le libellé reste celui de cet écran ; `path` désigne le champ dans le
 * document d'expérience et fait remonter la description et la contrainte que le
 * moteur publie. Sans `path` — ou sans descripteur correspondant — le champ rend
 * exactement ce qu'il rendait avant.
 */
function Field({ label, path, wide, children }: { label: string; path?: string; wide?: boolean; children: React.ReactNode }) {
  return <ConfiguredField label={label} path={path} wide={wide}>{children}</ConfiguredField>;
}
