      ...
      <div className="config-cards">
        {document.categories.map((category) => (
          <article key={category.id}>
            ...
            <label>Nom<input value={category.name} onChange={(event) => update((value) => ({ ...value, categories: value.categories.map((item) => item.id === category.id ? { ...item, name: event.target.value } : item) }))} /></label>
            <label>Description<textarea value={category.description} onChange={(event) => update((value) => ({ ...value, categories: value.categories.map((item) => item.id === category.id ? { ...item, description: event.target.value } : item) }))} /></label>
            <div className="studio-inline-fields">
              <label>Accent
            ...
            <AssetField
              ...
              label="Illustration" value={category.imageUrl ?? ""}
            ...
            />
            <label>Identifiants de scénarios (séparés par des virgules)
      ...
      <div className="config-cards">
        {document.journeys.map((journey) => (
          <article key={journey.id}>
            ...
            <label>Nom<input value={journey.name} onChange={(event) => update((value) => ({ ...value, journeys: value.journeys.map((item) => item.id === journey.id ? { ...item, name: event.target.value } : item) }))} /></label>
            <label>Description<textarea value={journey.description} onChange={(event) => update((value) => ({ ...value, journeys: value.journeys.map((item) => item.id === journey.id ? { ...item, description: event.target.value } : item) }))} /></label>
...
export function FamiliarSection({ document, update, manifest, packAbsentReason }: {
  ...
  return (
    <div className="studio-panel studio-split">
      <div className="studio-form">
        <label>Nom<input value={familiar.name} onChange={(event) => patch({ name: event.target.value })} /></label>
        <label>Description<textarea value={familiar.description} onChange={(event) => patch({ description: event.target.value })} /></label>
        <div className="studio-inline-fields">
          <label>Forme
          ...
          <label>Style d’écriture<input value={familiar.writingStyle} onChange={(event) => patch({ writingStyle: event.target.value })} /></label>
        </div>
        <label>Accent
        ...
        <label>Niveau d’aide : {familiar.helpLevel}/5
        ...
        <AssetField
          ...
          label="Portrait" value={familiar.portraitUrl ?? ""} onChange={(next) => patch({ portraitUrl: next || undefined })}
        />
        <div className="studio-inline-fields">
          <label>Licence<input value={familiar.license ?? ""} onChange={(event) => patch({ license: event.target.value || undefined })} /></label>
          <label>Attribution<input value={familiar.attribution ?? ""} onChange={(event) => patch({ attribution: event.target.value || undefined })} /></label>
        </div>
      </div>
      <figure className="surface-preview">
